...
module.exports.login = function(req, res, next) {
  ...
  User.findOne( { email: req.body.email.trim().toLowerCase() ,  } , { _id : 1 , name : 1 , email : 1 , password : 1  } ).exec(function(err, user) {
    if (err) {
      return next(err);
    }
...
module.exports.signup = function(req, res, next) {
  ...
  User.findOne({ email:{ $eq: req.body.email } }, function(err, user){
    ...
    if (user == null){
      ...
      Employee.findOne({ email:{ $eq: req.body.email } }, function(err, employee){
        if (err)
      throw err;
    if (employee){
